...
test('parameter screen keeps filtered export, nested scenario writes and audit history', async () => {
  ...
  assert.match(screen, /AdminSelectFilter label="Categoría" value={schema\.title}/)
  assert.match(screen, /selectEntityByTitle/)
  assert.match(screen, /itemLabel="registros configurados"/)
...
test('plans expose complete validated CRUD, metrics, details and read-only RBAC', async () => {
  ...
  for (const token of ['existingPlans', 'data.modules', 'crypto.randomUUID()', 'AHORA_ADMIN', 'Number.isFinite', 'precio_mensual', 'dias_prueba', 'orden_visualizacion', 'nivel_soporte', 'renovacion_automatica_default', 'modules', 'limits', 'CONTACTOS_MENSUALES', 'INFORMES_MENSUALES', 'toggleModule', 'type="checkbox"', 'Stepper', 'min-h-11', 'htmlFor=', 'role="alert"']) assert.match(dialog, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
