...
import { AdminUserFullDetail } from './AdminUserFullDetail'
...
export function AdminUsersScreen() {
  ...
  if (detail && detail.kind !== 'company') return <AdminUserFullDetail target={detail} onClose={() => setDetail(null)} />
  return <><AdminPageHeader title="Usuarios" description="Gestiona empresas, colaboradores, postulaciones y cuentas desde un solo lugar." /><div className="grid grid-cols-1 gap-4 md:grid-cols-3">{kpis.map((item) => <AdminKpiCard key={item.title} item={item} />)}</div><AdminCard className="mt-5"><AdminTabs ariaLabel="Tipos de usuario" value={tab} onChange={selectTab} items={[{ value: 'companies', label: 'Empresas', count: data.companies.length }, { value: 'collaborators', label: 'Colaboradores', count: data.collaborators.length }, { value: 'applications', label: 'Solicitudes', count: data.applications.length }, { value: 'tour', label: 'Usuarios sin empresa', count: counts.noCompany }]}>
    <div className="p-4">{tab === 'companies' ? <><div className="flex flex-wrap items-center justify-between gap-3"><h2>Empresas</h2><AdminButton variant="primary" onClick={() => setRegistration('company')}><Plus aria-hidden="true" size={16} />Registrar empresa</AdminButton></div>{filterBar('Plan', ['Todos', ...uniqueValues(data.companies, 'estado')], ['Todos', ...uniqueValues(data.companies, 'plan')], 'Empresas', ['Empresa', 'RUC', 'Ciudad', 'Plan', 'Suscripción', 'Registro'], companyRows.map((row) => [row.nombre_comercial, row.ruc, row.ciudad, row.plan, row.subscriptionState, row.created_at]))}<AdminDataTable rows={companyRows} columns={companyColumns} rowKey={(row) => row.id} caption="Empresas administradas" itemLabel="usuarios con empresa" renderActions={(row) => <AdminButton size="icon" variant="ghost" aria-label="Ver empresa" onClick={() => setDetail({ kind: 'company', record: row })}><Eye aria-hidden="true" size={16} /></AdminButton>} /></> : null}
    {tab === 'collaborators' ? <><div className="flex flex-wrap items-center justify-between gap-3"><h2>Colaboradores</h2><AdminButton variant="primary" onClick={() => setRegistration('collaborator')}><Plus aria-hidden="true" size={16} />Registrar colaborador</AdminButton></div>{filterBar('Especialidad', ['Todos', ...uniqueValues(data.collaborators, 'estado')], ['Todos', ...Array.from(new Set(data.collaborators.flatMap((item) => item.specialties))).sort((a, b) => a.localeCompare(b, 'es'))], 'Colaboradores', ['Profesión', 'Área', 'Modalidad', 'Tarifa', 'Disponibilidad', 'Estado'], collaboratorRows.map((row) => [row.profesion, row.area_especializacion, row.modalidad_atencion, row.tarifa_referencial, row.estado_disponibilidad, row.estado]))}<AdminDataTable rows={collaboratorRows} columns={collaboratorColumns} rowKey={(row) => row.id} caption="Colaboradores administrados" itemLabel="cuentas profesionales" renderActions={(row) => <AdminButton size="icon" variant="ghost" aria-label="Ver colaborador" onClick={() => setDetail({ kind: 'collaborator', record: row })}><Eye aria-hidden="true" size={16} /></AdminButton>} /></> : null}
    {tab === 'applications' ? <><h2>Solicitudes profesionales</h2>{filterBar('Especialidad', ['Todos', ...uniqueValues(data.applications, 'estado')], ['Todos', ...uniqueValues(data.applications, 'especialidad_principal')], 'Solicitudes', ['Profesional', 'Correo', 'Área', 'Modalidad', 'Estado', 'Solicitud'], applicationRows.map((row) => [`${row.nombres} ${row.apellidos}`, row.correo, row.area_especializacion, row.modalidad_atencion, row.estado, row.created_at]))}<AdminDataTable rows={applicationRows} columns={applicationColumns} rowKey={(row) => row.id} caption="Solicitudes profesionales" itemLabel="postulaciones registradas" renderActions={(row) => <AdminButton size="icon" variant="ghost" aria-label="Revisar solicitud" onClick={() => setApplication(row)}><Eye aria-hidden="true" size={16} /></AdminButton>} /></> : null}
    {tab === 'tour' ? <><h2>Usuarios sin empresa</h2>{filterBar('Ciudad', ['Todos', ...uniqueValues(allTourRows, 'estado')], ['Todos', ...uniqueValues(allTourRows, 'ciudad')], 'Usuarios sin empresa', ['Usuario', 'Correo', 'Ciudad', 'Correo verificado', 'Estado'], tourRows.map((row) => [`${row.nombres} ${row.apellidos}`, row.correo, row.ciudad, row.correo_verificado ? 'Verificado' : 'Pendiente', row.estado]))}<AdminDataTable rows={tourRows} columns={tourColumns} rowKey={(row) => row.id} caption="Usuarios sin empresa" itemLabel="cuentas aún sin empresa" renderActions={(row) => <AdminButton size="icon" variant="ghost" aria-label="Ver usuario" onClick={() => setDetail({ kind: 'user', record: row })}><Eye aria-hidden="true" size={16} /></AdminButton>} /></> : null}</div>
  </AdminTabs></AdminCard><AdminUserDetailDrawer target={detail?.kind === 'company' ? detail : null} onClose={() => setDetail(null)} /><AdminApplicationReviewDialog application={application} onClose={() => setApplication(null)} /><AdminRegistrationDialog kind={registration} open={Boolean(registration)} onClose={() => setRegistration(null)} /></>
}
