...
function SafeDocument({ value, label }: { value: string | null; label: string }) { return value && esUrlAdminPermitida(value) ? <a className="document-link min-h-11 inline-flex items-center gap-1" href={value} target="_blank" rel="noreferrer">{label}<ExternalLink aria-hidden="true" size={13} /></a> : <span>—</span> }
...
export function AdminApplicationReviewDialog({ application, onClose }: { application: ApplicationRecord | null; onClose: () => void }) {
  ...
  const reject = () => {
    ...
    if (!trimmed) { setError('Escribe un motivo de rechazo antes de continuar.'); requestAnimationFrame(() => reasonRef.current?.focus()); return }
    setReviewing(true)
  ...
  return <AdminDialog open={Boolean(application)} title={application ? `${application.nombres} ${application.apellidos}` : 'Solicitud profesional'} description="Revisa la información antes de validar al profesional." onClose={onClose} wide footer={application && pending ? <><AdminButton variant="danger" disabled={reviewing} onClick={reject}>Rechazar</AdminButton><AdminButton variant="success" disabled={reviewing} onClick={approve}>Aprobar profesional</AdminButton></> : <AdminButton onClick={onClose}>Cerrar</AdminButton>}>
    {application ? <div className="application-review">
      ...
      {pending ? <div className="form-field"><label htmlFor={reasonId}>Motivo de rechazo</label><textarea ref={reasonRef} id={reasonId} aria-describedby={error ? `${reasonId}-error` : undefined} value={reason} onChange={(event) => { setReason(event.target.value); setError('') }} placeholder="Escribe una razón clara para comunicarla al profesional." />{error ? <p id={`${reasonId}-error`} role="alert">{error}</p> : null}</div> : application.motivo_rechazo ? <section className="review-section"><h3>Motivo de rechazo</h3><p>{application.motivo_rechazo}</p></section> : null}
    </div> : null}
  </AdminDialog>
}
