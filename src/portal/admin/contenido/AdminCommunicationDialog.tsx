...
export function AdminCommunicationDialog({ item, open, onClose }: { item: CommunicationRecord | null; open: boolean; onClose: () => void }) {
  ...
  const [action, setAction] = useState<'BORRADOR' | 'PROGRAMADA' | 'PUBLICADA'>('BORRADOR')
  const formRef = useRef<HTMLFormElement>(null)
  const titleRef = useRef<HTMLInputElement>(null)
  const baseId = useId().replace(/:/g, '')
  useEffect(() => { if (open) { setForm(item ? { ...item, channels: [...item.channels] } : blankCommunication()); setError(''); setSaving(false); setAction('BORRADOR') } }, [item, open])
  ...
  const save = (status: 'BORRADOR' | 'PROGRAMADA' | 'PUBLICADA') => (event: FormEvent<HTMLFormElement>) => {
