...
export function TutorialesScreen() {
  ...
  const filtrados = useMemo(() => {
    ...
    return VIDEO_TUTORIALES.filter(
      (t) =>
  ...
  const hayMas = cantidad < VIDEO_TUTORIALES.length && categoria === 'Todos' && !busqueda.trim()

  return (
    <section className="flex flex-col gap-4.5 pb-4">
      <div>
        <h1 className="text-[28px] font-bold leading-tight">Video tutoriales</h1>
        <p className="mt-1.5 text-[14.5px] text-ink-700">
          Aprende a usar SAFE con tutoriales prácticos y paso a paso.
        </p>
      </div>
