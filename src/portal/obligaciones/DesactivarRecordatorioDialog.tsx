import { X } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { useAccessibleDialog } from '@/portal/plan/useAccessibleDialog'

export function DesactivarRecordatorioDialog({
  abierto,
  onCerrar,
  onConfirmar,
}: {
  abierto: boolean
  onCerrar: () => void
  onConfirmar: () => void
}) {
  const { dialogRef, titleRef } = useAccessibleDialog(abierto, onCerrar)

  if (!abierto) return null

  const confirmar = () => {
    onConfirmar()
    onCerrar()
  }

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        className="animate-safe-fade-in absolute inset-0 bg-navy-900/65 backdrop-blur-sm"
        aria-hidden="true"
        onMouseDown={(event) => {
          event.preventDefault()
          onCerrar()
        }}
      />
      <div
        ref={dialogRef}
        role="dialog"
        aria-modal="true"
        aria-labelledby="desactivar-recordatorio-titulo"
        className="animate-safe-pop-in relative w-full max-w-[400px] rounded-2xl border border-line/70 bg-card p-5 shadow-[var(--shadow-float)]"
      >
        <div className="flex items-start justify-between gap-3">
          <h2
            ref={titleRef}
            id="desactivar-recordatorio-titulo"
            tabIndex={-1}
            className="text-lg font-semibold text-ink-900 outline-none"
          >
            Desactivar recordatorio
          </h2>
          <button
            type="button"
            onClick={onCerrar}
            className="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-ink-500 transition-colors hover:bg-surface hover:text-ink-900"
            aria-label="Cerrar"
          >
            <X className="h-4.5 w-4.5" aria-hidden="true" />
          </button>
        </div>
        <p className="mt-2 text-sm leading-relaxed text-ink-700">
          Dejarás de recibir avisos antes del vencimiento de esta obligación. ¿Deseas continuar?
        </p>
        <div className="mt-5 flex justify-end gap-2.5">
          <Button variant="outline" onClick={onCerrar}>
            Volver
          </Button>
          <Button className="bg-destructive text-white hover:bg-destructive/90" onClick={confirmar}>
            Desactivar
          </Button>
        </div>
      </div>
    </div>
  )
}
